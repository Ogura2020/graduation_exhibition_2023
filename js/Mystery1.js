class Mystery1 extends Dialog {
  ...
  constructor(selector) {
      ...
      this.nazoDialog.addEventListener('click', () => {
        if (this.nazoDialog.classList.contains('mystery1')) {
          console.log('mystery1');
          this.tips.src = 'img/mystery1/hint.png';
...
onStart(e) {
  ...
  if (nextOpenState) {
    this.nazoDialog.classList.add('mystery1');
  } else {
    this.nazoDialog.classList.remove('mystery1');
  }
  ...
  if (this.dialog) {
    if (!this.isOpen && nextOpenState) {
      ...
      // ダイアログ内の画像を更新
      this.dialog.querySelector('.mystery_img').src = "img/mystery1/paper0.png"; // ここに新しい画像のパスを設定(初めに表示される画像)
...
onReadRFID(e) {
  ...
  if (this.isOpen) {
    console.log('[mystery1] onReadRFID', e);
    ...
    if (this.tips.style.display === "block") {
      ...
      this.imgElement.src = "img/mystery1/" + this.imageList[0];
    } else if (e === this.status[2].id) {
      this.sound.select_2.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[1];
    } else if (e === this.status[3].id) {
      this.sound.select_3.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[2];
    } else if (e === this.status[4].id) {
      this.sound.select_4.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[3];
    } else if (e === this.status[5].id) {
      this.sound.select_5.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[4];
    } else if (e === this.status[6].id) {
      this.sound.select_6.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[5];
    } else if (e === this.status[7].id) {
      this.sound.select_7.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[6];
    } else if (e === this.status[8].id) {
      this.sound.select_8.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[7];
    } else if (e === this.status[9].id) {
      this.sound.select_9.play();
      this.imgElement.src = "img/mystery1/" + this.imageList[8];
    }
  }
}
  ...
  if (this.isOpen) {
    ...
    if (Math.abs(accel[0]) > 1.0 &&  this.tips.style.display === "block") {
      ...
      if (this.select !== this.id){
        ...
        if (!this.isMiss) {
          //関係ないカード以外は不正解音を鳴らす
          if(this.id !== this.status[0].id && this.id !== this.status[10].id){
