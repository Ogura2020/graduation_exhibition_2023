class Mystery8 extends Dialog {
  ...
  constructor(selector) {
      console.log('[mystery8] constructor ');
      super(selector);
      ...
      this.nazoDialog.addEventListener('click', () => {
        if (this.nazoDialog.classList.contains('mystery8')) {
          console.log('mystery8');
          this.tips.src = 'img/mystery8/hint.png';
...
onStart(e) {
  ...
  // 画像ファイル名のリスト
  this.imageList = ['do.png', 're.png', 'mi.png', 'fa.png', 'so.png', 'ra.png', 'si.png', 'do2.png', 're2.png', 'mi2.png', 'fa2.png']; 
  // ismystery が true のカットであるかの真偽値
  const nextOpenState = e.hasOwnProperty('ismystery8') && e.ismystery8;

  if (nextOpenState) {
    this.nazoDialog.classList.add('mystery8');
  } else {
    this.nazoDialog.classList.remove('mystery8');
  }
  ...
  if (this.dialog) {
    if (!this.isOpen && nextOpenState) {
      ...
      // ダイアログ内の画像を更新
      this.dialog.querySelector('.mystery_img').src = "img/mystery8/0.png"; // ここに新しい画像のパスを設定(初めに表示される画像)
...
onReadRFID(e) {
  ...
  if (this.isOpen) {
    console.log('[mystery8] onReadRFID', e);
    ...
    if (this.tips.style.display === "block") {
      ...
      this.imgElement.src = "img/mystery8/" + this.imageList[0];
    } else if (e === this.status[1].id) {
      this.sound.select_1.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[1];
    } else if (e === this.status[2].id) {
      this.sound.select_2.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[2];
    } else if (e === this.status[3].id) {
      this.sound.select_3.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[3];
    } else if (e === this.status[4].id) {
      this.sound.select_4.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[4];
    } else if (e === this.status[5].id) {
      this.sound.select_5.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[5];
    } else if (e === this.status[6].id) {
      this.sound.select_6.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[6];
    } else if (e === this.status[7].id) {
      this.sound.select_7.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[7];
    } else if (e === this.status[8].id) {
      this.sound.select_8.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[8];
    } else if (e === this.status[9].id) {
      this.sound.select_9.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[9];
    } else if (e === this.status[10].id) {
      this.sound.select_10.play();
      this.imgElement.src = "img/mystery8/" + this.imageList[10];
    }
  }
}
...
onReadAccel(e) {
  ...
  if (this.isOpen) {
    console.log('[mystery8] onReadAccel', e);
    this.ee.emit('select', e);
    ...
    } else if (Math.abs(accel[0]) > 1.0 && !this.imgElement.src.endsWith(this.imageList[4]) && this.imgElement.src.endsWith("img/mystery8/0.png")) {
