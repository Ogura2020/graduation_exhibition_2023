class Mystery11 extends Dialog {
  /**
   * constructor
  ...
  constructor(selector) {
      console.log('[mystery11] constructor ');
      super(selector);
      ...
      this.nazoDialog.addEventListener('click', () => {
        if (this.nazoDialog.classList.contains('mystery11')) {
          console.log('mystery11');
          this.tips.src = 'img/mystery11/hint.png';
...
onStart(e) {
  ...
  const nextOpenState = e.hasOwnProperty('ismystery11') && e.ismystery11;

  if (nextOpenState) {
    this.nazoDialog.classList.add('mystery11');
  } else {
    this.nazoDialog.classList.remove('mystery11');
  }
  ...
  if (this.dialog) {
    if (!this.isOpen && nextOpenState) {
      ...
      // ダイアログ内の画像を更新
      this.dialog.querySelector('.mystery_img').src = "img/mystery11/0.png"; // ここに新しい画像のパスを設定(初めに表示される画像)
...
onReadRFID(e) {
  ...
  if (this.isOpen) {
    console.log('[mystery11] onReadRFID', e);
    ...
    if (this.tips.style.display === "block") {
      ...
      this.imgElement.src = "img/mystery11/" + this.imageList[0];
    } else if (e === this.status[1].id) {
      this.sound.select_1.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[1];
    } else if (e === this.status[2].id) {
      this.sound.select_2.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[2];
    } else if (e === this.status[3].id) {
      this.sound.select_3.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[3];
    } else if (e === this.status[4].id) {
      this.sound.select_4.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[4];
    } else if (e === this.status[5].id) {
      this.sound.select_5.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[5];
    } else if (e === this.status[6].id) {
      this.sound.select_6.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[6];
    } else if (e === this.status[7].id) {
      this.sound.select_7.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[7];
    } else if (e === this.status[8].id) {
      this.sound.select_8.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[8];
    } else if (e === this.status[9].id) {
      this.sound.select_9.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[9];
    } else if (e === this.status[10].id) {
      this.sound.select_10.play();
      this.imgElement.src = "img/mystery11/" + this.imageList[10];
    }
  }
}
...
onReadAccel(e) {
  ...
  if (this.isOpen) {
    console.log('[mystery11] onReadAccel', e);
    this.ee.emit('select', e);

    let accel = e.split(',')
    console.log(accel[0], accel[1], accel[2]);

    
    // if (Math.abs(accel[0]) > 0 ) {
    //   console.log("x" +accel[0])
    // } 
    // if (Math.abs(accel[1]) > 0 ) {
    //   console.log("Y" + accel[1])
    // } 
    // if (Math.abs(accel[2]) > 0 ) {
    //   console.log("z" +accel[2])
    // }


    if ((Math.abs(accel[0]) > 1.0 || Math.abs(accel[1]) > 0.2) &&  this.tips.style.display === "block") {
      console.log("ヒント表示中（block）の時は何も起こらない")
    } else if ((Math.abs(accel[0]) > 1.0 || Math.abs(accel[1]) > 0.2) && this.imgElement.src.endsWith(this.imageList[2])) {
    ...
    } else if ((Math.abs(accel[0]) > 1.0 || Math.abs(accel[1]) > 0.2) && !this.imgElement.src.endsWith(this.imageList[2]) && this.imgElement.src.endsWith("img/mystery11/0.png")) {
